...
function isCJKorFullwidth(grapheme: string): boolean {
...
const _measCache = new Map<string, number>();
const MEAS_CACHE_MAX = 4_000;

// Layer 2: CJK square validation — keyed by font
const _squareCache = new Map<string, boolean>();

// Layer 3: full prepared data — keyed by font + text
const _prepCache = new Map<string, PreparedVerticalText>();
const PREP_CACHE_MAX = 2_000;
...
function measureGraphemeWidth(grapheme: string, font: string, fontSize: number): number {
...
function cjkIsSquare(font: string, fontSize: number): boolean {
  let valid = _squareCache.get(font);
  if (valid !== undefined) return valid;
  const ctx = getCtx(font);
  if (!ctx) { _squareCache.set(font, true); return true; }
...
function graphemeAdvanceH(grapheme: string, font: string, fontSize: number): number {
  if (!grapheme || grapheme === " " || grapheme === "\u3000") return fontSize;
  if (isCJKorFullwidth(grapheme) && cjkIsSquare(font, fontSize)) return fontSize;
 ...
 */
export function prepareVertical(
  text: string,
  font: string,
  options?: PrepareVerticalOptions,
): PreparedVerticalText {
  ...
  const segments: string[]              = new Array(len);
  const advanceHeights: number[]        = new Array(len);
  const canBreakBefore: boolean[]       = new Array(len);
  const breakableAdvances: (number[] | null)[] = new Array(len);
  ...
  const prepared: PreparedVerticalText = {
    segments,
    advanceHeights,
    canBreakBefore,
    breakableAdvances,
...
export const CURSOR_START: VerticalCursor = { segmentIndex: 0, graphemeIndex: 0 };
 ...
 */
function stepColumn(
  prepared: PreparedVerticalText,
  startCursor: VerticalCursor,
  maxHeight: number,
  ...
  // Bounds check
  if (segIdx >= totalSegs) return null;
  ...
  for (let i = segIdx; i < totalSegs; i++) {
    ...
    if (columnH + adv > maxHeight) {
      ...
      for (let j = fStart; j < forcedEnd; j++) forcedH += advanceHeights[j];
      return {
        height: forcedH,
        start: startCursor,
...
function materializeText(prepared: PreparedVerticalText, range: LayoutColumnRange): string {
 ...
 */
export function layoutNextColumn(
  prepared: PreparedVerticalText,
 ...
 */
export function layoutNextColumnRange(
  prepared: PreparedVerticalText,
  cursor: VerticalCursor,
  maxHeight: number,
): LayoutColumnRange | null {
...
export function layoutVertical(
  ...
  maxHeight: number,
  columnWidth: number,
): LayoutVerticalResult {
  let count = 0;
  let cursor: VerticalCursor = CURSOR_START;
  while (true) {
    const range = stepColumn(prepared, cursor, maxHeight);
    if (range === null) break;
    count++;
 ...
 */
export function layoutVerticalWithColumns(
  prepared: PreparedVerticalText,
  maxHeight: number,
  columnWidth: number,
): LayoutVerticalWithColumnsResult {
 ...
 */
export function walkColumnRanges(
  prepared: PreparedVerticalText,
  maxHeight: number,
  onColumn: (column: LayoutColumnRange) => void,
): number {
  let count = 0;
  let cursor: VerticalCursor = CURSOR_START;
  while (true) {
 ...
 */
export function setVerticalLocale(locale?: string): void {
  _locale = locale ?? "ja";
  _segmenter = null;
 ...
 */
export function clearVerticalLayoutCaches(): void {
  _measCache.clear();
  _squareCache.clear();
  _prepCache.clear();
  _ctxCache.clear();
